...
def extractArticlePublishedDate(articleLink, html=None):
    ...
    try:
        ...
        if html is None:
            ...
            # html = extractHTMLfromNewsPaper(articleLink)
            # articleDate = extractPossibleDate(html)
...
def extractHTMLfromNewsPaper(url):
    nparticle = newspaper.Article(url)
    nparticle.download()
    nparticle.parse()
    return nparticle.html


if __name__ == "__main__":
    d = extractArticlePublishedDate(
        "https://www.marketscreener.com/quote/stock/JULS-BAER-76954418/news/JULS-BAER-Swiss-banker-to-Venezuelan-kleptocrats-becomes-star-witness-32917574/"
    )
    print(d)
